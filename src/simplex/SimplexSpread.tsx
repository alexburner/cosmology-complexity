...
import { drawGraphsAndShells, getPoints } from "./util"

const CANVAS_W = 600
const CANVAS_H = 44
const CENTER = new paper.Point(CANVAS_H / 2, CANVAS_H / 2)
const RADIUS = Math.round(CANVAS_H * 0.4 * 0.98)
...
export const SimplexSpread: FC<{ n: number }> = ({ n }) => {
...
const draw = (args: { n: number; canvas: HTMLCanvasElement }) => {
  ...
  if (args.n > 0) {
    ...
    Object.values(graphsAndShells).forEach((lines, i) => {
      lines.forEach((line) => {
        line.position.x += (i + 0) * RADIUS * 3
        group.addChild(line)
      })
    })
  }
